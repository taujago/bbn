...
function index(){
	...
	$data_array['arr_leasing'] = 
	$this->cm->arr_dropdown("M_LEASING","LEASING_ID","LEASING_NAMA","LEASING_NAMA");
	$content = $this->load->view("verifikasi_view",$data_array,true);
		
	$this->set_subtitle("VERIFIKASI DATA BPKB");
	$this->set_title("VERIFIKASI DATA BPKB");
...
function simpan(){
	...
	if($data['IS_VERIFIED'] == "1" and $data['STATUS'] <> 'AKTIF') {
		$ret = array("error"=>true,"message"=>"STATUS KENDARAAN TIDAK BISA UNTUK DIVERIFIKASI");
	}
	else {
